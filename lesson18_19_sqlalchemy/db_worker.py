...
class DatabaseWorker:
    ...
    async def update_post_by_id(self, post_id: int, **data):
        if not data:
        ...
        async with self._session as s:
            ...
            """
            async with s.begin():
                post = await self._get_post_by_id_from_session(s, post_id=post_id)
                """
                NOTE: if we would use *self.get_user_by_id(user_id=user_id)* here,
                the updates we will make bellow will not be applied to real Database state!
                Because in this case the User object was got within the another Session
                ...
                if (title := data.get("title")) is not None:
                    post.title = title  # update user's name
                if (description := data.get("description")) is not None:
                    post.description = description  # update user's age
                if (user_id := data.get("user_id")) is not None:
                    post.user_id = user_id  # update user's gender
                ...
                *lazy="joined"* on "posts" attribute in User class.
                ...
                to the User object within the Session
                """
                if (updated_comments := data.get("updated_comments")) is not None:
                    for updated_comment in updated_comments:
                        await s.merge(updated_comment)


                if (new_comment := data.get("new_comment")) is not None:
                    post.comment.append(new_comment)

                if (new_like := data.get("new_like")) is not None:
                    post.like.append(new_like)
